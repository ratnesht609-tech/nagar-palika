...
import { FileText, Scroll, ClipboardList, BookOpen, Send, Sparkles, MessageSquare, FileUp, Landmark, ShieldCheck, PenTool, Megaphone, ReceiptText, FileWarning } from 'lucide-react';
...
export const DraftSelector: React.FC<Props> = ({ onSelect, onAIStart, onChatStart, onPromptStart }) => {
  return (
    <div className="max-w-6xl mx-auto p-4 animate-fade-in">
      ...
      <div className="mb-10 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* NEW: Prompt based AI Drafting */}
        <div className="bg-gradient-to-br from-indigo-700 to-indigo-600 rounded-3xl p-6 text-white shadow-xl flex flex-col justify-between relative overflow-hidden group border-b-4 border-indigo-400">
          <div className="relative z-10">
             <div className="flex items-center gap-2 mb-3">
               <span className="bg-white/20 text-white text-[10px] font-black px-2 py-1 rounded-full uppercase tracking-widest border border-white/30">QUICK DRAFT</span>
               <h2 className="text-xl font-bold font-serif-hindi">विवरण से ड्राफ्ट</h2>
             </div>
             <p className="text-indigo-100 text-sm mb-6 leading-relaxed">
               सिर्फ विषय या संक्षिप्त विवरण लिखें, AI पूरा आधिकारिक पत्र तैयार कर देगा।
             </p>
          </div>
          <button 
            onClick={onPromptStart}
            className="relative z-10 bg-indigo-900 text-white hover:bg-indigo-950 px-6 py-3 rounded-2xl font-black text-sm shadow-xl transition-all flex items-center gap-2 w-fit active:scale-95 border border-indigo-400/30"
          >
            <PenTool size={18} />
            लिखकर बताएं
          </button>
      ...
      <div className="mt-12 p-6 bg-indigo-50/50 rounded-3xl border border-indigo-100 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-indigo-700/80 text-xs leading-relaxed font-medium italic">"यह प्रणाली उत्तर प्रदेश नगर पालिका अधिनियम, 1916 के विधिक परामर्श पर आधारित है। एआई द्वारा जनरेट किए गए प्रारूपों का विधिक सत्यापन अनिवार्य है।"</p>
        </div>
      </div>
    </div>
  );
